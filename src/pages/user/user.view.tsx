...
export default function UserView() {
  ...
  // Calculate how many users would actually be deleted
  const usersToDeleteCount = userType === "user" 
    ? users.filter(u => u.id !== userId).length 
    : users.length;
  
  // Show "Deletar Todos" only if there are users to delete
  const showDeleteAllButton = canDelete && usersToDeleteCount > 0;
      ...
      <div className="max-w-7xl mx-auto space-y-4 md:space-y-6">
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:justify-between sm:items-start">
          <div className="flex-1">
            <h1 className="text-2xl md:text-3xl font-bold text-indigo-900">Usuários</h1>
            <p className="text-sm md:text-base text-muted-foreground mt-1">Visualize e gerencie todos os usuários da sua empresa</p>
          </div>
          {showDeleteAllButton && (
            <Button onClick={handleDeleteAll} variant="destructive" className="w-full sm:w-auto" disabled={isDeleting}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
          <Card className="border-0 shadow-md hover:shadow-lg transition-shadow">
            <CardContent className="p-4 md:p-6">
              <div className="flex items-center justify-between">
                <div><p className="text-xs md:text-sm font-medium text-muted-foreground mb-1">Total de Usuários</p><p className="text-2xl md:text-3xl font-bold text-indigo-900">{isLoading ? <Skeleton className="h-8 w-12" /> : users.length}</p></div>
                <div className="w-10 h-10 md:w-12 md:h-12 rounded-lg bg-indigo-100 flex items-center justify-center"><Users className="w-5 h-5 md:w-6 md:h-6 text-indigo-600" /></div>
              </div>
            </CardContent>
          </Card>
          <Card className="border-0 shadow-md hover:shadow-lg transition-shadow">
            <CardContent className="p-4 md:p-6">
              <div className="flex items-center justify-between">
                <div><p className="text-xs md:text-sm font-medium text-muted-foreground mb-1">Administradores</p><p className="text-2xl md:text-3xl font-bold text-purple-600">{isLoading ? <Skeleton className="h-8 w-12" /> : adminCount}</p></div>
                <div className="w-10 h-10 md:w-12 md:h-12 rounded-lg bg-purple-100 flex items-center justify-center"><Shield className="w-5 h-5 md:w-6 md:h-6 text-purple-600" /></div>
              </div>
            </CardContent>
          </Card>
          <Card className="border-0 shadow-md hover:shadow-lg transition-shadow sm:col-span-2 lg:col-span-1">
            <CardContent className="p-4 md:p-6">
              <div className="flex items-center justify-between">
                <div><p className="text-xs md:text-sm font-medium text-muted-foreground mb-1">Usuários Regulares</p><p className="text-2xl md:text-3xl font-bold text-slate-600">{isLoading ? <Skeleton className="h-8 w-12" /> : regularCount}</p></div>
                <div className="w-10 h-10 md:w-12 md:h-12 rounded-lg bg-slate-100 flex items-center justify-center"><Users className="w-5 h-5 md:w-6 md:h-6 text-slate-600" /></div>
              </div>
            </CardContent>
          </Card>
        </div>
        ...
        <Card className="border-0 shadow-md">
          <CardHeader className="pb-3 md:pb-4">
            <div className="flex flex-col gap-1">
              ...
              <CardDescription className="text-xs md:text-sm">{isLoading ? "Carregando..." : `${users.length} ${users.length === 1 ? "usuário cadastrado" : "usuários cadastrados"}`}</CardDescription>
            </div>
          </CardHeader>
          <CardContent className="p-0 md:p-6">
            {isLoading && users.length === 0 ? (
              ...
              <div className="text-center py-12 px-4">
                ...
                <p className="text-sm md:text-base text-muted-foreground">Nenhum usuário encontrado</p>
              </div>
            ) : (
              <>
                    ...
                    <TableBody>
                      {users.map((usuario) => (
                        <TableRow key={usuario.id} className="hover:bg-slate-50/50">
                          <TableCell>
                            <div className="flex items-center gap-3">
                              <div className="w-10 h-10 rounded-full bg-gradient-to-br from-indigo-100 to-purple-100 flex items-center justify-center shadow-sm">
                                <span className="text-indigo-700 font-semibold text-sm">{usuario.nome?.charAt(0).toUpperCase() || "U"}</span>
                              </div>
                          ...
                          <TableCell>
                            <Badge variant={usuario.grupo?.nome === "admin" ? "default" : "secondary"} className={usuario.grupo?.nome === "admin" ? "bg-purple-100 text-purple-700 hover:bg-purple-100" : "bg-slate-100 text-slate-700 hover:bg-slate-100"}>
                          ...
                          <TableCell>
                            <div className="flex items-center gap-2 text-gray-600">
                              ...
                              <span className="text-sm">{usuario.dataCriacao ? new Date(usuario.dataCriacao).toLocaleDateString("pt-BR") : "N/A"}</span>
                            </div>
                          </TableCell>
                          {canDelete && (
                            <TableCell className="text-right">
                              {userType === "user" && usuario.id === userId ? (
                                ...
                                <Button variant="ghost" size="sm" onClick={() => handleDeleteUser(usuario.id)} className="text-red-600 hover:text-red-700 hover:bg-red-50">
                      ...
                      <CardContent className="p-4">
                        <div className="flex items-start gap-3 mb-4">
                          <div className="w-12 h-12 rounded-full bg-gradient-to-br from-indigo-100 to-purple-100 flex items-center justify-center shadow-sm flex-shrink-0">
                            <span className="text-indigo-700 font-semibold text-base">{usuario.nome?.charAt(0).toUpperCase() || "U"}</span>
                          </div>
                          <div className="flex-1 min-w-0">
                            <h3 className="font-semibold text-gray-900 truncate mb-1">{usuario.nome}</h3>
                            <Badge variant={usuario.grupo?.nome === "admin" ? "default" : "secondary"} className={usuario.grupo?.nome === "admin" ? "bg-purple-100 text-purple-700 text-xs" : "bg-slate-100 text-slate-700 text-xs"}>
                              {usuario.grupo?.nome || "Pendente"}
                            </Badge>
                          </div>
                        </div>
                        <div className="space-y-2.5 pt-3 border-t border-gray-100">
                          <div className="flex items-center gap-2 text-sm"><Mail className="w-4 h-4 text-gray-400 flex-shrink-0" /><span className="text-gray-600 truncate">{usuario.email}</span></div>
                          <div className="flex items-center gap-2 text-sm"><Calendar className="w-4 h-4 text-gray-400 flex-shrink-0" /><span className="text-gray-600">{usuario.dataCriacao ? new Date(usuario.dataCriacao).toLocaleDateString("pt-BR") : "N/A"}</span></div>
                          {canDelete && (userType !== "user" || usuario.id !== userId) && (
                            <Button variant="outline" size="sm" onClick={() => handleDeleteUser(usuario.id)} className="w-full text-red-600 hover:text-red-700 hover:bg-red-50 border-red-200 mt-2">
                              <Trash2 className="w-4 h-4 mr-2" />Deletar Usuário
                            </Button>
                          )}
                        </div>
                      </CardContent>
