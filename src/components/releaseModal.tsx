...
import { formatCurrencyInput, parseCurrencyBR, formatNumberBR } from "@/utils/formatters";
...
const ReleaseModal = ({
	...
	useEffect(() => {
		if (release) {
			setFormData({
				...
				valor: release.notaFiscal?.valor ? formatNumberBR(release.notaFiscal.valor) : "",
	...
	const handleSave = async () => {
		...
		try {
			...
			// Diagnóstico para empresas (debug)
			const { userType: currentUserType, userId: currentUserId } = useAuthStore.getState();
			if (currentUserType === "enterprise") {
				console.log("🏢 Executando diagnóstico para empresa antes do upload...");
